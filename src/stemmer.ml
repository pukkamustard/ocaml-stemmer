...
let rule_list_1b =
  [(105, "eed", "ee", 0); (106, "ed", "", -1); (107, "ing", "", -1)]
...
let ends_with_cvc str =
  ...
  if len < 3 then false
    ...
    (not (vowel_or_wxy str.[len - 1]))
    && vowel_or_y str.[len - 2]
    && not (is_vowel str.[len - 3])
  then true
  else false
...
let rules_criteria =
  [([106; 107; 123], contains_vowel); ([122], add_an_e); ([502], remove_an_e)]
...
let match_rule word ((num, orig, _, min_root) : int * string * string * int) =
  ...
  if word_len > orig_len then
    ...
    if
      ...
      && aux_rule word_root num rules_criteria
    then
      (*print_int num;
